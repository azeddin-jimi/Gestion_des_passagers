        ...
        <div class="row align-items-center g-4">
            <div class="col-lg-5">
                ...
                <p class="lead opacity-90 mb-0">
                    {{ __('Trouvez un trajet en quelques secondes avec une recherche simple et rapide.') }}</p>
            </div>
            <div class="col-lg-7">
                        ...
                        <form action="{{ route('trajets.search') }}" method="get" class="row g-3" data-loading>
                            <div class="col-md-6">
                                ...
                                <select name="departure_city" id="departure_city"
                                    class="form-select @error('departure_city') is-invalid @enderror" required>
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="arrival_city" id="arrival_city"
                                    class="form-select @error('arrival_city') is-invalid @enderror" required>
                            ...
                            <div class="col-12 d-grid">
                                <button type="submit" class="btn btn-markoub btn-lg btn-animated">
                                    ...
                                    <span class="spinner-border spinner-border-sm ms-2 d-none" role="status"
                                        aria-hidden="true" data-spinner></span>
                                </button>
                            </div>
                        </form>
                    </div>
                        ...
                        <div class="card border-0 shadow-sm h-100 transition-card card-hover">
                            <div class="card-body">
                                <div class="d-flex justify-content-between align-items-start mb-2">
                                    ...
                                    <span class="badge text-bg-light text-dark border">{{ $trajet->seats_available }}
                                        {{ __('pl.') }}</span>
                                </div>
                                ...
                                <p class="h5 mb-3">{{ number_format($trajet->price, 2, ',', ' ') }} <span
                                        class="small text-muted">MAD</span></p>
                                @auth
                                    @if ($trajet->isBookable())
                                        <a href="{{ route('reservations.create', $trajet) }}"
                                            class="btn btn-outline-primary btn-sm w-100">{{ __('Réserver') }}</a>
                                    @else
                                        <button class="btn btn-outline-secondary btn-sm w-100" disabled>{{ __('Complet') }}</button>
                                    @endif
                                @else
                                    <a href="{{ route('login') }}"
                                        class="btn btn-markoub btn-sm w-100">{{ __('Connexion pour réserver') }}</a>
                                @endauth
                            </div>
                        </div>
                    </div>
