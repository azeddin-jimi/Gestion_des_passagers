            ...
            <form action="{{ route('reservations.store', $trajet) }}" method="post" class="d-grid gap-4" data-loading>
                ...
                <div class="card border-0 shadow-sm" data-aos="fade-up">
                    <div class="card-body p-4">
                        <h2 class="h5 mb-3"> {{ __('Contact') }}</h2>
                        <div class="row g-3">
                            <div class="col-md-6">
...
<option value="+49" @selected(old('country_code') === '+49')>🇩🇪 +49</option>
<option value="+34" @selected(old('country_code') === '+34')>🇪🇸 +34</option>

<option value="+971" @selected(old('country_code') === '+971')>🇦🇪 +971</option>
<option value="+966" @selected(old('country_code') === '+966')>🇸🇦 +966</option>
<option value="+965" @selected(old('country_code') === '+965')>🇰🇼 +965</option>
<option value="+974" @selected(old('country_code') === '+974')>🇶🇦  +974</option>
<option value="+973" @selected(old('country_code') === '+973')>🇧🇭 +973</option>
<option value="+968" @selected(old('country_code') === '+968')>🇴🇲 +968</option>

<option value="+20" @selected(old('country_code') === '+20')>🇪🇬 +20</option>
<option value="+216" @selected(old('country_code') === '+216')>🇹🇳 +216</option>
<option value="+213" @selected(old('country_code') === '+213')>🇩🇿 +213</option>

<option value="+221" @selected(old('country_code') === '+221')>🇸🇳 +221</option>
<option value="+225" @selected(old('country_code') === '+225')>🇨🇮  +225</option>
<option value="+234" @selected(old('country_code') === '+234')>🇳🇬  +234</option>
<option value="+27" @selected(old('country_code') === '+27')>🇿🇦  +27</option>

<option value="+91" @selected(old('country_code') === '+91')>🇮🇳 +91</option>
<option value="+86" @selected(old('country_code') === '+86')>🇨🇳 +86</option>
<option value="+64" @selected(old('country_code') === '+64')>🇳🇿 +64</option>
                                </select>
                            ...
                            <div class="col-md-8">
                                ...
                                <input type="text" name="whatsapp"
                                    ...
                                    value="{{ old('whatsapp', auth()->user()->whatsapp) }}" placeholder="XXXXXXXXXX"
                                    required>
                ...
                <div class="card border-0 shadow-sm" data-aos="fade-up" data-aos-delay="80">
                    <div class="card-body p-4">
                        <h2 class="h5 mb-2"> {{ __('Réservation') }}</h2>
                        <p class="small text-muted">
                            {{ __('Placement libre à bord. Choisissez le nombre de places souhaité.') }}</p>
                        <label class="form-label">{{ __('Nombre de places') }}</label>
                        <input type="number" name="seats_reserved" id="seats_reserved"
                            class="form-control @error('seats_reserved') is-invalid @enderror"
                            value="{{ old('seats_reserved', 1) }}" min="1" max="{{ $trajet->seats_available }}" required>
                    ...
                    <div class="card-body p-4">
                        <div class="form-check mb-2">
                            <input class="form-check-input @error('terms_accepted') is-invalid @enderror" type="checkbox"
                                name="terms_accepted" value="1" id="terms_accepted" @checked(old('terms_accepted'))
                                required>
                        ...
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="newsletter_opt_in" value="1"
                                id="newsletter_opt_in" @checked(old('newsletter_opt_in'))>
        ...
        <div class="col-lg-4">
            <div class="card border-0 shadow-sm position-sticky" style="top: 90px;" data-aos="fade-left">
                <div class="card-body p-4">
                    ...
                    <p class="small text-muted mb-1">{{ $trajet->date->format('d/m/Y') }}
                        {{ \Illuminate\Support\Str::substr($trajet->time, 0, 5) }}</p>
                    <p class="small text-muted">{{ __('Voyageurs') }}: <span id="summarySeats">{{ $initialSeats }}</span>
                    </p>

                    <div class="mb-3">
                        <label class="form-label small">{{ __('Code de réduction') }}</label>
                        <input type="text" name="discount_code"
                            class="form-control form-control-sm @error('discount_code') is-invalid @enderror"
                            value="{{ old('discount_code') }}" placeholder="PROMO2026">
                    ...
                    <div class="small d-grid gap-2">
                        <div class="d-flex justify-content-between">
                            <span>{{ __('Ticket') }}</span>
                            <span id="summaryTicket">{{ number_format($ticketPrice * $initialSeats, 2, ',', ' ') }}
                                DH</span>
                        </div>
                        <div class="d-flex justify-content-between">
